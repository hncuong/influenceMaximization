...
double LTSimplePath::mineSeedSet() {
	...
	multimap<double,UID>::iterator it = covQueue.end();
	it--;
    ...
    while (curSeedSet.size() < budget) {
		...
		//cout<<"Compare Result:"<<node_to_compare<<" "<<MG_to_compare<<endl;
        ...
        for (UserList::iterator i = T.begin(); i != T.end(); ++i) {
			...
			//cout<<x<<" "<<covSMap.find(x)->second<<"+"<<cov_X<<"-"<<totalCov<<"="<<xMG<<endl;
        } //endfor
		//cout<<"Max Result:"<<winnerID<<" "<<winnerMG<<endl;
